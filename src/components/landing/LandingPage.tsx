    ...
    <div className="min-h-screen bg-mesh font-body text-ink">
      <header className="sticky top-0 z-50 border-b border-ink/5 bg-cream/75 backdrop-blur-xl">
        <div className="mx-auto flex h-20 max-w-[1280px] items-center justify-between px-6 lg:px-12">
          ...
          <div className="flex items-center gap-3"><Link to="/auth" className="hidden text-sm text-ink/70 hover:text-ink sm:block">Sign in</Link><Link to="/dashboard" className="rounded-full bg-ink px-5 py-2.5 text-sm font-semibold text-cream hover:bg-ink/90">Create escrow</Link></div>
        </div>
      </header>
          ...
          <div className="mx-auto grid max-w-[1280px] items-center gap-14 px-6 pb-24 pt-16 lg:grid-cols-2 lg:px-12 lg:py-28">
            <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }}>
              ...
              <div className="mt-9 flex flex-wrap gap-4">
                <Link to="/dashboard/escrows" className="group inline-flex items-center gap-2 rounded-full bg-ink px-7 py-4 font-semibold text-cream shadow-lg shadow-ink/10">Start an escrow <ArrowRight size={17} className="transition-transform group-hover:translate-x-1" /></Link>
